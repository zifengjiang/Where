...
- 最终产品决策为始终同一平面：左侧“场景 / 所有物品”成组，右侧独立圆形加号。使用 iOS 26 action-tab 拦截方案，不使用会在 expanded 状态形成第二行的 `tabViewBottomAccessory`。
...
- 点击添加后立即进入 Apple 系统相机，不显示自定义来源对话框；模拟器/无相机设备直接进入系统照片选择器。取消系统界面后安全回到录入页，仍可重新拍摄或从相册选择。
